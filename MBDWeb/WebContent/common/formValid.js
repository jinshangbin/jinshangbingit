...
Validator = {
	...
	Mobile : /^1([3|4|5|7|8]){1}\d{9}$/,//手机验证
	//PhoneMobile : /^(0?1([3|4|5|7|8]){1}\d{9})$|^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7})$/,
	...
	Jin : /^[ a-zA-Z0-9]+$/, //匹配英文数字和空格
	EnglishSec: /^[A-Za-z0-9\'\"\s\.\(\)\[\]\{\}<>\?\\\/\,]*$/,//英文字母大小写以及特殊符号
	...
	ErrorMessage : [ $.i18n.prop("i18n.customer.msg.common")+"：\t\t\t\t" ],
	Validate : function(formId, mode) {
		...
		for ( var i = 0; i < count; i++) {
			with (obj.elements[i]) {
				...
				var _errorMsgAry;
				if(getAttribute("validi18n") != undefined && getAttribute("validi18n") != ""){
					_errorMsgAry = $.i18n.prop(getAttribute("validi18n")).split('|');
				}else{
				    _errorMsgAry = getAttribute("msg").split('|');
				}
