  ...
  $(".city_box").on("click",".ulhdDown li",function(e) {
      ...
      if(index==0){
    	...
    	if(pval==null || pval==''){
      		...
      		post('area/querylist','pid='+pval,"callback_clickarea",$(this));
      	}
      }else if(index==3){
          $(this).parents().siblings(".ulArea").hide().siblings(".ulArea3").show();
        	var pval = $(findNode($(e.target),".areaval",2)[0]).val();
      	if(pval==null || pval==''){
        	}else{
        		post('area/querylist','pid='+pval,"callback_clickZhen",$(this));
        	}
        }
  });
...
function  callback_clickarea(data,thisobj){
	var ul = $(findNode(thisobj,"ul.ulArea2",3)[0]);
		ul.empty();
		$.each(data,function(i,o){
			ul.append("<li value='"+o.id+"' onclick=\"clicktoZhen(this,'"+o.id+"','"+o.nameCn+"');return false;\">"+o.nameCn+"</li>");
		});
}


function callback_clickZhen(data,thisobj){
	var ul = $(findNode(thisobj,"ul.ulArea3",3)[0]);
	if(ul.children().length==0){
		ul.empty();
		$.each(data,function(i,o){
			ul.append("<li value='"+o.id+"' onclick=\"clickZhen(this,'"+o.id+"','"+o.nameCn+"');return false;\">"+o.nameCn+"</li>");
		});
	}
}
...
function clicktoZhen(thisobj,id,name){
	var ulp = $(thisobj).parent().parent();
	var p = ulp.find(".areaval");
	p.val(id);
	p.attr("inittext",name);
	findNode(ulp,".zhen",3).val("");
	findNode(ulp,".zhen",3).attr("inittext",'');
	$(findNode($(thisobj),".city_box",4).find("input")[0]).val(getText(thisobj));
	$(findNode(ulp,".ulhdDown li",4)[3]).click();
}


function clickZhen(thisobj,id,name){
...
function getText(thisobj){
	...
	var zhen = findNode($(thisobj),".zhen",3).attr("inittext");
	if(zhen==null || zhen==''){
		return resu;
	}else{
		resu = resu+"-"+zhen;
